...
def create_plane(rows,cols):
    ...
    for r in range(rows):
        ...
        plane["seat_map"].append(s)
    return plane

def get_number_economy_sold(plane):
    """
    Input: a dicitonary containing the number of regular economy seats sold. 
           the keys are the names for the tickets and the values are how many
    ...
    """
    sold = 0
    for v in plane["economy_sold"].values():
        sold = sold + v
    return sold


def get_avail_seats(plane):
    """
    Parameters: plane : a list of lists representing plaine
    ...
    """
    """avail = 0;
    for r in plane["seat_map"]:
        for c in r:
            if c == "avail" or c == "win":
                avail = avail + 1
    avail = avail - get_number_economy_sold(plane) - 
    return avail
    """

    return get_total_seats(plane) - get_number_economy_sold(plane) - plane["plus_sold"]
...
def get_total_seats(plane):
    ...
    Returns: The total number of seats in the plane
    """
    return len(plane['seat_map'])*len(plane['seat_map'][0])
...
def purchase_economy_plus(plane,name):
    """
    Params: plane - a list of lists representing a plane
            economy_sold - a dictionary representing the economy sold but not assigned
            name - the name of the person purchasing the seat
    """
    rows = len(plane['seat_map'])
    cols = len(plane['seat_map'][0])

    
    # total unassigned seats
    seats = get_avail_seats(plane)
    ...
    if random.randrange(100) > 30:
        ...
        for row in order:
            if plane['seat_map'][row][0] == "win":
                plane['seat_map'][row][0] = name
                return plane
            elif plane['seat_map'][row][len(plane['seat_map'][0])-1] == "win":
                plane['seat_map'][row][len(plane['seat_map'][0])-1] = name
                return  plane
    ...
    while not(found_seat):
        ...
        if plane['seat_map'][r_row][r_col] == "win" or plane['seat_map'][r_row][r_col] == "avail":
            plane['seat_map'][r_row][r_col] = name
            found_seat = True
    return plane


# THIS WILL BE LEFT EMPTY FOR THE FIRST STAGE OF THE PROJECT
def seat_economy(plane,name):
    """
...
def purchase_economy_block(plane,number,name):
    """
    Purchase regular economy seats. As long as there are sufficient seats
    available, store the name and number of seats purchased in the
    economy_sold dictionary and return the new dictionary
    ...
    seats_avail = seats_avail - get_number_economy_sold(plane)

    if seats_avail >= number:
        plane["economy_sold"][name]=number
    return plane["economy_sold"]
...
def fill_plane(plane):
    ...
    while total_seats > 1:
        ...
        if r > 30:
            plane = purchase_economy_plus(plane,"ep-%d"%ep_number)
            ep_number = ep_number + 1
            total_seats = get_avail_seats(plane)
        else:
            plane['economy_sold'] = purchase_economy_block(plane,1+random.randrange(max_family_size),"u-%d"%u_number)
            u_number = u_number + 1
    ...
    for name in plane['economy_sold'].keys():
        for i in range(plane['economy_sold'][name]):
            plane = seat_economy(plane,name)
